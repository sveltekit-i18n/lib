...
it. Of the parser only its TYPES (`Parser`, `Modifier`, `Report`) are
re-exported: its factory is wired internally, and its build-time parameter
extractor is not part of this surface yet.
...
`sveltekit-i18n` does **not** re-export a value from the parser: the parser
factory is wired internally and there is nothing to construct. Nor does it
export a parameter extractor yet — `BaseParser.ExtractParams` describes the
build-time contract a schema generator will be written against.
...
Until then the schema is hand-written for a small project, or generated by your
own build step for a large one.
